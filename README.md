...
Raspberry Pi Zero W 向け GPS ロガーを構築するための `pi-gen` カスタムステージです。  
このリポジトリは **`pi-gen-custom/stage2/10-gps-logger` の公開を前提**としています。

## 公開ポリシー

- 公開対象: `pi-gen-custom/stage2/10-gps-logger`
- `pi-gen` 本体は含めず、公式リポジトリを別途取得して利用
- ユーザー名/パスワードは記載例を含みますが、**必ず変更**してください
...
- AP の SSID / パスワードはビルドごとにランダム生成されます（固定の例示値は提示しません）。
- 生成結果ファイル:
...
- 公開前に秘密情報スキャン（例: gitleaks）を推奨
- 実運用ではデフォルト資格情報を使わないこと
